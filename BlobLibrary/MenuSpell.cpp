...
MenuSpell::MenuSpell(float width, float height, Blob &b)
	: Menu(width, height, b.getKnownSkills().size() + 1)
{
	...
	for (int i = 0; i < item_number - 1; i++) {
		...
		text[i].setPosition(sf::Vector2f(width / 2, height / (item_number + 1) * (i+1)));
	}
	...
	text[item_number - 1].setPosition(sf::Vector2f(width / 2, height / (item_number + 1) * item_number));
